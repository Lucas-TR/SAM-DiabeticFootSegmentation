...
def denoise(gray):
    ...
    try:
        ...
        mask2 = cv2.cvtColor(gray, cv2.COLOR_GRAY2BGR)*255
...
def denoise(gray):
    ...
    # Definir el kernel para la operación de morfología
    kernel = np.ones((3, 3), np.uint8)

    # Aplicar la operación de apertura para suavizar los contornos
    opening = cv2.morphologyEx(closed_mask, cv2.MORPH_OPEN, kernel)

    # Eliminar regiones pequeñas utilizando la transformación de área abierta
    cleaned = cv2.morphologyEx(opening, cv2.MORPH_OPEN, kernel, iterations=3)
