...
def feature_slic(name_experiment, y_val, y_val_mask, n_segment, compactness, sigma, threshold, layer, path_img, path_out, boolean, boolean_2, exp_all=False):
    RGB = False
    # load the model
    model = VGG16()
    ...
    for i in range(n_iter):
        ...
        if (boolean):
            var_ch_img_aux = var_ch_img[-channels:]
            me_ch_img_aux = me_ch_img[-channels:]

            df_aux = tool.create_df_feature_color(var_ch_img_aux, me_ch_img_aux, wound, name, x, y)
