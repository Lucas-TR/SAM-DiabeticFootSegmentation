...
def feature_slic(name_experiment, n_segment , compactness, sigma, threshold , layer, path_img , path_mask, path_out, boolean, boolean_2,exp_all=False):
  ...
  # Verificación de los tipos de path_img y path_mask, y codificación si son cadenas
  y_val, y_val_mask = tool.procesar_imagenes_y_mascaras(path_img, path_mask)
  ...
  #  if not(i in empty):
      ...
      if (boolean):
          ...
          PATH = path_out
          df_aux.to_csv("{}/{}/{}/{}_{}_VGG.csv".format(PATH,name_experiment,file_data_per_image,name_experiment, name_id[0]))
  ...
  PATH = path_out
  df.to_csv("{}/{}/{}/{}_feature_vector_VGG.csv".format(PATH,name_experiment,vector_feature_all,name_experiment))
  df.to_excel("{}/{}/{}/{}_feature_vector_VGG.xlsx".format(PATH,name_experiment,vector_feature_all,name_experiment))
  
  
  if exp_all:
      df.to_csv("{}/{}/{}_feature_vector_VGG.csv".format(PATH,'Data_training',name_experiment))
  ...
  with open (README, 'w') as f:
    f.write('Experiment performed using VGG\n \n INPUTS \n\n')
...
def feature_slic_prueba(name_experiment, n_segment , compactness, sigma, threshold , layer, path_img , path_out, boolean, boolean_2,exp_all=False):
  ...
  #  if not(i in empty):
      ...
      if (boolean):
          var_ch_img_aux = var_ch_img[-channels:]; me_ch_img_aux = me_ch_img[-channels:]
          
          df_aux = tool.create_df_feature_color(var_ch_img_aux, me_ch_img_aux, name, x, y,bool_label=False)
