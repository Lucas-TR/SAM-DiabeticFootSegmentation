...
<-----TOOL General----->
'''
def procesar_imagenes_y_mascaras(path_img, path_mask):
    """
    Procesa las rutas de las imágenes y las máscaras. Si las rutas son cadenas, utiliza la herramienta 'tool' para codificarlas.
    De lo contrario, devuelve las rutas tal como están.

    :param path_img: Ruta de la imagen o imagen directamente.
    :param path_mask: Ruta de la máscara o máscara directamente.
    :param tool: Herramienta con método 'encoder' para procesar las rutas.
    :return: Tupla con la imagen y la máscara procesadas.
    """
...
def create_df_feature_color(var_,med_,names,x_c_,y_c_,label='',bool_label=True):
...
def show_points_on_image(image, coords, labels, marker_size=15, marker_color=(0, 255, 0), marker_type=cv2.MARKER_STAR, complete= False):

    # Assure that coords are integers
    ...
    # Draw positive points on the image
    for point in pos_points:
        cv2.drawMarker(image, tuple(point), marker_color, marker_type, marker_size)

    # Change the color for negative points
    if complete:
        marker_color = (0, 0, 255) # Red color in BGR
    else:
        marker_color = (255, 0, 0) # Red color in BGR

    # Draw negative points on the image
    for point in neg_points:
        cv2.drawMarker(image, tuple(point), marker_color, marker_type, marker_size)

    return image

def segment_image(image,input_point,input_label,  input_box, predictor):
    """
    Genera la segmentación de una imagen utilizando un modelo SAM.
...
def overlay_mask(image, mask, mask_color=(0, 255, 0), alpha=0.5):
...
def filtro_r_specular(img):
    # Convert the image to HSV, RGB and CIE-Lab color spaces
    hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)
    rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    lab = cv2.cvtColor(img, cv2.COLOR_BGR2LAB)

    # Normalize the saturation, green and lightness channels to 0-1 range
    ...
    green_pixels = img[:,:,1]
    x1 = np.mean(green_pixels)
    exponente = exp_green(x1)
    
    # Calculate the feature image using the given equation
    feature_img = ((1 - saturation) * green * lightness)**exponente

    # Apply a standard deviation filter of size 3 to the feature image
    filtered_img = cv2.GaussianBlur(feature_img, (3,3), cv2.BORDER_DEFAULT)

    # Calculate the standard deviation of each pixel in the filtered image
    mean, std_dev = cv2.meanStdDev(filtered_img)

    # Normalize the filtered image to 0-1 range
    normalized_img = (filtered_img - mean) / std_dev

    # Threshold the normalized image using a threshold value of 0.9
...
#relación logarítmica que representa el cambio de escala entre la escala 1 que va desde 8 a 200 
#y la escala 2 que va desde 3 a 10 es:
