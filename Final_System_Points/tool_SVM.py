...
def separate_data_train(df, predict = False):
    # Loading data
    #df.head()
...
def my_sample_points(model_RL, df, df_x_y_var_mean, k):
    probabilities = model_RL.predict_proba(df_x_y_var_mean)

    indices_0 = np.argsort(probabilities[:, 0])[::-1]
    indices_1 = np.argsort(probabilities[:, 1])[::-1]

    df_ = df[:, [0, 1]]
    data_negative = df_[indices_0]
    data_positive = df_[indices_1]

    return data_positive[:k, :], data_negative[:2*k, :]

def prediction_SVM_predict(path_out_origin, path_out, path_img,n_segment, compactness, sigma, threshold , layer, name_image, method,predictor):
    
    if method == 1: 
        file_models = "model_SVM_RGB"
    if method == 2: 
        file_models = "model_SVM_VGG16"
    if method == 3: 
    ...
    for i in range(n_iter):
        ...
        if method == 1: 
            name_experiment = "Results_RGB"
            df = vector_slic_rgb.feature_slic_prueba(name_experiment, n_segment, compactness, sigma, threshold, path_img, PATH,file_per_image,file_iou_image)
        if method == 2: 
            name_experiment = "Results_VGG16"
        ...
        #Load model
        model = joblib.load("{}/{}/{}".format(path_out_origin,file_models,data_models[0])) 
        ...
        selected_points, selected_points_n = my_sample_points(model, df.values,  X, 5)
...
def create_img_final(img, segments_slic, clas, name_image, Path_out):
    img = img[:,:,(2,1,0)]
    marcadores = []
    for i in clas:
        marcadores.append(np.where(segments_slic==i))
    ...
    #creamos la máscara usando superpixeles
    vista_1 = np.zeros(img.shape, dtype="uint8")
    for idx in range(len(marcadores)):
        vista_1[marcadores[idx]] = 255

    vista_2 = cv2.addWeighted(vista_1,0.4,img,0.6,0)
    vista_3 = cv2.bitwise_and(img, vista_1)
        
    filename_1 = '{}/{}_1.jpg'.format(Path_out, name_image)
    filename_2 = '{}/{}_2.jpg'.format(Path_out, name_image)
    filename_3 = '{}/{}_3.jpg'.format(Path_out, name_image)
